...
// Lightweight fallback for Tolgee's `useTranslate` and `T` component.
// This allows the app to run without wrapping the tree in TolgeeProvider.
// If you intend to use Tolgee for i18n, re-add the provider and restore imports.
const defaultStrings = {
  "bmi-calculator-title": "BMI Calculator",
  "bmi-calculator-height-label": "Height (m)",
  "bmi-calculator-weight-label": "Weight (kg)",
  "bmi-calculator-height-placeholder": "e.g. 1.75",
  "bmi-calculator-weight-placeholder": "e.g. 70",
  "bmi-calculator-calculate-button": "Calculate",
  "bmi-calculator-reset-button": "Reset",
  "bmi-calculator-your-bmi": "Your BMI is {bmi}",
  "bmi-calculator-underweight": "Underweight",
  "bmi-calculator-normal-weight": "Normal weight",
  "bmi-calculator-overweight": "Overweight",
  "bmi-calculator-obese": "Obese",
  "bmi-calculator-invalid-input": "Please enter valid height and weight",
};

const useTranslateFallback = () => {
  const t = (key, params) => {
    let template = defaultStrings[key] || key;
    if (params) {
...
const TFallback = ({ keyName, params, children }) => {
  const { t } = useTranslateFallback();
  // If children provided, render them (keeps compatibility with some usages)
  ...
  return (
    <div className="w-full max-w-sm p-6 bg-white rounded-lg shadow-lg text-center transition-all duration-300 hover:shadow-xl">
      <h1 className="text-2xl font-bold mb-4 transition-transform duration-300 hover:scale-105">
        <TFallback keyName="bmi-calculator-title" />
      </h1>
      ...
      <form onSubmit={calculateBMI} className="mb-4">
        <div className="mb-4 transition-all duration-300">
          <label className="block text-left font-medium text-gray-700 mb-1 transition-colors duration-300">
            <TFallback keyName="bmi-calculator-height-label" />
          </label>
          <input
            type="number"
            value={height}
        ...
        <div className="mb-6 transition-all duration-300">
          <label className="block text-left font-medium text-gray-700 mb-1 transition-colors duration-300">
            <TFallback keyName="bmi-calculator-weight-label" />
          </label>
          <input
            type="number"
            value={weight}
        ...
        >
          <TFallback keyName="bmi-calculator-calculate-button" />
        </button>
      </form>
        ...
        >
          <div className="bg-gradient-to-r from-blue-50 to-green-50 p-6 rounded-lg border-2 border-gray-100 shadow-inner">
            <div className="flex items-center justify-center mb-4">
              <div
                className={`text-4xl font-bold ${getBMIColor()} transition-all duration-500 transform hover:scale-110`}
              >
                {bmi}
              </div>
            </div>
            ...
            <p className="text-lg font-semibold mb-2 transition-colors duration-300">
              <TFallback keyName="bmi-calculator-your-bmi" params={{ bmi }} />
            </p>
            ...
            >
              <TFallback keyName="bmi-calculator-reset-button" />
            </button>
          </div>
        </div>
      )}
